...
use signal_frame::{
    BoundExchangeFrame, ExchangeFrameBody, ExchangeIdentifier, ExchangeLane, LaneSequence, Reply,
...
impl OrdinaryClient {
    ...
    pub fn from_arguments(arguments: impl IntoIterator<Item = OsString>) -> Result<Self> {
        let input =
            Potential::<signal_lojix::Request>::from(single_inline_datom_argument(arguments)?)
    ...
    pub fn input(&self) -> &signal_lojix::Request {
        &self.input
    }
    ...
    pub fn run(self) -> Result<signal_lojix::Response> {
        ...
        let frame = BoundExchangeFrame::<
            signal_lojix::LojixWire,
            signal_lojix::RequestWire,
            signal_lojix::ResponseWire,
        >::decode_length_prefixed(&reply)?;
        match frame.into_body() {
            ExchangeFrameBody::Reply {
                exchange: found,
                reply,
            } if found == exchange => match reply {
                Reply::Accepted { per_operation, .. } => match per_operation.into_head() {
                    SubReply::Ok(wire)
                    | SubReply::Failed {
                        detail: Some(wire), ..
                    } => signal_lojix::Response::try_from_wire(wire)
...
pub struct MetaClient {
    input: meta_signal_lojix::Request,
}
...
impl MetaClient {
    ...
    pub fn from_arguments(arguments: impl IntoIterator<Item = OsString>) -> Result<Self> {
        let input =
            Potential::<meta_signal_lojix::Request>::from(single_inline_datom_argument(arguments)?)
                .actualize(IncorporationBudget::try_from(16_384).expect("positive request budget"))
                .map_err(|fault| Error::DatomRequestText(format!("{fault:?}")))?;
        Ok(Self { input })
    }

    pub fn input(&self) -> &meta_signal_lojix::Request {
        &self.input
    }
    ...
    pub fn run(self) -> Result<meta_signal_lojix::Response> {
        ...
        let request = BoundExchangeFrame::<
        ...
        >::new(
            signal_frame::WireRoute::new(
                ...
                signal_frame::VariantCode::new(route),
            ),
            ExchangeFrameBody::Request {
                exchange,
                request: signal_frame::Request::from_payload(input.into_wire()),
            },
        )
        ...
        match frame.into_body() {
            ExchangeFrameBody::Reply {
                ...
                Reply::Accepted { per_operation, .. } => match per_operation.into_head() {
                    ...
                    | SubReply::Failed {
                        detail: Some(wire), ..
                    } => meta_signal_lojix::Response::try_from_wire(wire)
                        .map_err(|fault| Error::Wire(format!("{fault:?}"))),
                    _ => Err(Error::UnexpectedFrame),
                },
                Reply::Rejected { .. } => Err(Error::SignalRequestRejected),
